...
use crate::{cavern::Cavern, gamedata::{GameDataResource, cavern}, position::{Position, Layer}, actors::{Direction, Sprites, HorizontalMotion, Actor}};
...
fn spawn_guardians(
    ...
    cavern: ResMut<Cavern>, game_data: Res<GameDataResource>, mut images: ResMut<Assets<Image>>,
  ) {
  if cavern.is_changed() {
    println!("I would like to spawn guardians for cavern {}", cavern.cavern_number);
    println!("Guardians: {:?}", game_data.caverns[cavern.cavern_number].guardians);
    ...
    for (id, g) in cavern_data.guardians.iter().enumerate() {
        let images: Vec<_> = cavern_data.guardian_bitmaps
          .iter()
        ...
        commands.spawn(Actor::new(
          ...
          HorizontalMotion {
            walking: true,
            direction: if g.first_animation_frame <= 3 { Direction::Right } else { Direction::Left }
          }
        ));
    }
  }
