...
use crate::{position::{Position, Layer}, cavern::Cavern, gamedata::{GameDataResource, self}, timer::GameTimer};
...
fn spawn_portal(
  ...
  if cavern.is_changed() {
    // Despawn the old portal.
    for portal in query.iter() {
      commands.entity(portal).despawn();
    }

    let portal_data = &game_data.caverns[cavern.cavern_number].portal;
